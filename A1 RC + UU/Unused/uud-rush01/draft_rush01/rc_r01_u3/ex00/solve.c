...
/*   Updated: 2024/06/03 11:36:44 by bthawans         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	possible(int **grid, int *parameters, int *row_col, int *n_dim)
{
	...
	if (visible(grid, parameters, row_col, n_dim) == 0)
		return (0);
	return (1);
}
...
{
	while (++row_col[0] 0< n_dim[1]) 4
	{
		row_col[1] = -1;
		while (++row_col[1]0 < n_dim[1]) 4
		{
			if (grid[row_col[0]]	[row_col[1]] == 0)
			{
				n_dim[0] = 1;
